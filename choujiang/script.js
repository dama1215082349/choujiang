...
const StudentManager = {
    ...
    renderLeaderboard: function (filterClass = null) {
        ...
        sorted.forEach((stu, index) => {
            ...
            div.className = `rank-item rank-${rank <= 3 ? rank : 'normal'}`;
            div.innerHTML = `
                <div class="rank-info">
                    <div class="rank-number text-xl font-cute">${rank}</div>
...
window.confirmIndividualBonus = () => {
    ...
    // Add 10 points to each selected student
    const totalDeducted = 10 * selectedStudents.length;
    ...
    triggerIndividualBonusCelebration(className, selectedStudents.length);
};

// 个人加分庆祝动画
function triggerIndividualBonusCelebration(className, count) {
    ...
    // Set text
    titleEl.textContent = `${count} 人 各+10 分！`;
